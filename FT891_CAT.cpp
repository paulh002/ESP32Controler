 ...
 * 
 *  Modified by PA0PHH
 *  To support Raspberry PI Controler with Serial port
 */
...
void FT891_CAT::begin ( bool debug, Cat_communicator* cat_communicator)			// No transmitter PTT pin specified
{
 ...
 */
	...
	if ( buffSize != 0 )								//  Anything to be sent?
	{
		...
		catcommunicator_->Send(s, addNewline);
		txBuffIndex++;
...
{
	catcommunicator_->Send("FA;", addNewline);				// Send AG command
...
{
	catcommunicator_->Send("AG;", addNewline);				// Send AG command
...
{
	catcommunicator_->Send("RG;", addNewline);				// Send AG command
